...
const getBaseUrl = () => {
    // 1. Check for manual override (useful for testing or specific migrations)
    const overrideUrl = localStorage.getItem('abi_newBackendUrl');
    if (overrideUrl) return overrideUrl;

    const { hostname, port, protocol } = window.location;
    const isLocal = hostname === 'localhost' || hostname === '127.0.0.1' || hostname.startsWith('192.168.');

    // 2. Local Environment (Running via npm start/dev or Live Server)
    if (isLocal) {
        // If we're on port 5000, we're likely being served by the backend directly
        if (port === '5000') return '/api';
        // Otherwise, point to the local backend
        return 'http://localhost:5000/api';
    }

    // 3. Directly Served from Backend (e.g. Render/Railway serving everything)
    if (protocol !== 'file:' && hostname !== 'localhost') {
        /* 
           If you deploy the backend and frontend TOGETHER on one host, return '/api'.
           If you use Netlify for frontend, change the string below to your Render URL.
        */
        const productionBackendUrl = ''; // PLACE YOUR RENDER URL HERE (e.g., https://app.onrender.com)
        return productionBackendUrl ? `${productionBackendUrl}/api` : '/api';
    }

    // 4. Fallback for physical file access
    return 'http://localhost:5000/api';
};

const API_URL = getBaseUrl();
console.log(`[Abi_new] API initialized at: ${API_URL}`);

// Shared utility functions
const apiRequest = async (endpoint, options = {}) => {
    const token = localStorage.getItem('abi_newNodeToken');
    const headers = {
        ...
        if (contentType && contentType.includes("application/json")) {
            try {
                ...
                if (text.trim().startsWith('<!DOCTYPE html>') || text.trim().startsWith('<html')) {
                    const errorMsg = backendHeader === 'Abi_new-Node'
            ...
            if (text.trim().startsWith('<!DOCTYPE html>') || text.trim().startsWith('<html')) {
                ...
                if (!backendHeader) {
                    data.message = `CRITICAL: The backend server was NOT reached. Request was handled by: ${serverHeader || 'a static host'}. This usually means you deployed as a "Static Site" instead of a "Web Service". Ensure your deployment configuration routes /api to the Node server.`;
                } else {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (facultyForm) {
        facultyForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                localStorage.setItem('abi_newNodeToken', data.token);
                localStorage.setItem('abi_newUser', JSON.stringify(data));
    ...
    if (window.location.pathname.endsWith('/dashboard.html')) {
        const user = JSON.parse(localStorage.getItem('abi_newUser'));
...
// Logout Utility
function logout() {
    localStorage.removeItem('abi_newNodeToken');
    localStorage.removeItem('abi_newUser');
