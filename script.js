...
const getBaseUrl = () => {
    ...
    // If we're on localhost but NOT port 5000 (e.g. VS Code Live Server on 5500),
    // we should point to the backend at 5000.
    if (isLocal && port !== '5000') {
        return 'http://localhost:5000/api';
    }

    // If we're served from a server (like Render or localhost:5000)
    // we should use relative paths to avoid CORS and host issues
    if (protocol !== 'file:') {
        return '/api';
    }

    // If we're opening the HTML file directly on disk, we need to point to a running backend
    return 'http://localhost:5000/api';
};
